...
Because this is currently a snapshot, you will need to add the snapshot repository to the resolvers in build.sbt.
...
`allOf` and `anyOf` are convenience functions for creating a `List[Array]` and an `Array[String]`.  Older documentation and examples may refer to `as` and `la` - these have been deprecated in favour of the more descriptive `allOf` and `anyOf`, but functionality remains the same.
    ...
    @import be.objectify.deadbolt.core.utils.TemplateUtils.{allOf, anyOf}
...
2.5.0 Migration Guide
---------------------

- There is no longer a common module shared by the Scala and Java versions of Deadbolt.  This means some classes have moved package:
 - `be.objectify.deadbolt.core.models.` => `be.objectify.deadbolt.java.models.`
 - `be.objectify.deadbolt.core.PatternType` => `be.objectify.deadbolt.java.models.PatternType`
 - `be.objectify.deadbolt.core.utils.TemplateUtils` => `be.objectify.deadbolt.java.utils.TemplateUtils`
- `DeadboltPlugin` has been removed - delete its entry in `play.plugins`
- `CompletionStage<>` is now used by `DeadboltHandler` and `DynamicResourceHandler`.
- `TemplateUtils.as` and `TemplateUtils.la` have been deprecated, and usages of them should be replaced with `TemplateUtils.allOf` and `TemplateUtils.anyOf` respectively.
- `TemplateUtils.roles(Permission...)` was named incorrectly, and has been corrected to `TemplateUtils.permissions(Permission...)`
- Don't return `null` where a `CompletionStage` or `Optional` is required!
